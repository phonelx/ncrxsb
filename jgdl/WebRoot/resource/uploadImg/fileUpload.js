...
var uploadTools = {
    ...
    "initWithLayout":function(opt){
       ...
       /* if(opt.showSummerProgress){
        ...
        //添加文件显示框
        var boxStr = "<div class='box photoBox'></div>";
        $("#"+uploadId).append(boxStr);
        
      //选择文件和上传按钮模板
        var btsStr = "";
        btsStr += "<div class='uploadBts' align='center'><br/>";
        btsStr += "<button class='selectFileBt btn btn-primary' style='width:150px;'>选择文件</button>";
        //上传按钮
       /* if(!opt.isHiddenUploadBt){
            btsStr += "<span class='uploadFileBt'>";
            btsStr += "<button class='selectFileBt btn btn-primary'>上传</button>";
            btsStr += " </span>";
        }*/
        //清理按钮
        if(!opt.isHiddenCleanBt){
            btsStr += "<div class='cleanFileBt' style='margin-top:10px;'>";
            btsStr += "<button class='selectFileBt btn btn-primary' style='width:150px;'>清除</button>";
            btsStr += " </div>";
        }
        btsStr += "</div>";
