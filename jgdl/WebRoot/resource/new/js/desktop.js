...
//$(function(){
//	$("#myWinTab_showWSCatalog").live("click",function(){
//		alert("dianji");
//	});
//});
...
myLib.desktop.win={
	winHtml:function(title,url,id){
		...
		+title
		+"</b><span class='win_btnblock'><a href='javaScript:void(0);' class='winMinimize' title='最小化'></a><a href='javaScript:void(0);' class='winMaximize' title='最大化'></a><a href='javaScript:void(0);' class='winHyimize' title='还原'></a><a href='javaScript:void(0);' class='winClose' id='desktopCloseId' name='desktopCloseId' title='关闭'></a></span></div><iframe frameborder='0' name='myFrame_"
		+id
		+"' id='myFrame_"
		+id
    ...
    newWin:function(options){
        ...
        var defaults = {
            ...
            WindowWidth:          Math.round(wh['w']*0.3),           /* Only pixels */
            WindowHeight:         Math.round(wh['h']*0.4),           /* Only pixels */
		...
		//判断窗口位置，否则使用默认值
		var dxy= (curwinNum>10 ? curwinNum % 10 :  curwinNum )*5; //Math.floor((Math.random()*100))-90;
		var panelLeft=$("#desktopInnerPanel").position();
		//桂人杰 窗口起始位置
//		alert(myLib._is(options['WindowPositionLeft'],"Number") + " , "+
...
//		);
		...
		wLeft -= 40;
		var wTop=myLib._is(options['WindowPositionTop'],"Number")?options['WindowPositionTop']+dxy/2-70:(wh['h']-options['WindowHeight'])/2+dxy/2-70;
		//var wLeft=myLib._is(options['WindowPositionLeft'],"Number")?options['WindowPositionLeft']+dxy-panelLeft.left:(wh['w']-options['WindowWidth'])/2+dxy-panelLeft.left;
		//var wTop=myLib._is(options['WindowPositionTop'],"Number")?options['WindowPositionTop']+dxy/2-70:(wh['h']-options['WindowHeight'])/2+dxy/2-70;
		
		//给窗口赋予新的z-index值
		var zindex=curwinNum+99999;
		...
		winClose_btn.click(function(){
			...
			if(options["closeEvent"]!=null){
				options.closeEvent(options.WindowsId);   
			}
	...
	addWinTab:function(text,id){
		var myData=myLib.desktop.getMydata();
 		var $task_lb=myData.panel.taskBar['task_lb']
		    ...
		    ,docHtml="<a href='javaScript:void(0);'id='"+tid+"'>"+text+"</a>";
...
myLib.desktop.deskIcon={
	...
	arrangeIcons:function(desktop){
          var myData=myLib.desktop.getMydata()
             ...
             ,iconNum1=desktop.find("b").size();
			
		 //存储当前总共有多少桌面图标
		 desktop.data('deskIconNum',iconNum);
	//	 var deskIconheigt=$("#deskIcon0").height();
		 var deskIconwidth=$("#deskIcon0").width();
	//	 alert(deskIconwidth);
		 var gH=105;//一个图标总高度，包括上下margin
		 var gW=110;//图标总宽度,包括左右margin
		 var rows=8;
		 var rows1=Math.floor((deskIconwidth-75)/gW);
				   ...
				   "margin":0,
					"top":function(index,value){
					   ...
					   "left":function(index,value){
						   var v=(index-rows1*currow)*gW+20;
							if((index+1)%rows1==0){
								 currow=currow+1;
								}
					    return v;
	...
	desktopMove:function($innerPanel,$deskIcon,$navTab,dates,moveDx,nextIndex){
		 ...
		 .animate({
				 left:-(nextIndex)*moveDx
							...
							if(nextIndex==0){
				 				//点击首页
				 				$("#win1").removeClass().addClass("currIndex");
				 			}else{
				 				//点击子页
				 				$("#win1").removeClass().addClass("currIndex1");
				 			}
	...
	init:function(iconData){
		 ...
		 .click(function(){
			 var data=$(this).data("iconData"),id=this.id;
			 if(this.id=="scdsj"||this.id=="zgxxcx"){
