...
public class ProjectConfigDaoImpl implements IProjectConfigDao {
	...
	*/
	...
	@Override
	public Project getProjectInfoBySqu(String projectSqu,Connection conn) {
		ResultSet rs = null;
		Project project = new Project();
		String sql = " SELECT XMBH,XMDZ,XMMC,DZSFLD,DZJSD,DZLX,ALPHAMAX FROM PROJECT WHERE PROJECT_SQU = '"+projectSqu+"' ";
		try {
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			while(rs.next()){
				project.setXmbh(rs.getString("XMBH"));
				project.setXmdz(rs.getString("XMDZ"));
				project.setXmmc(rs.getString("XMMC"));
				project.setDzsfld(rs.getString("DZSFLD"));
				project.setDzjsd(rs.getString("DZJSD"));
				project.setDzlx(rs.getString("DZLX"));
				project.setAlphaMax(rs.getDouble("ALPHAMAX"));
			}
		} catch (SQLException e) {
