package monitor.goodsHouse.bean.vo;
/**
 *@ClassName
 *@dataTime 2017-10-17-下午8:34:36
 *@version
 *@author:唐青
 *@since
 */
public class CpmxbVo {
	private	String	CpSQU;  
	private	String	CPBM;
	private	String	CYMC;
	private	String	BJXH;
	private	String	JLDW;
	private	String	CBDJ;
	private	String	CpPARENTSQU;
	private	String	EDHL;
	private	String	CPXH;
	private	String	CPXL;
	private	String	CPZM;
	private	String	CPTZ;
	private	String	ZP;
	private	String	LXCS;

		//代码类别
		private String XHLB;
		//类别名称
		private String XHMC;
		//代码类别
		private String XLLB;
		//类别名称
		private String XLMC;
		//代码类别
		private String ZMLB;
		//类别名称
		private String ZMMC;
		//代码类别
		private String TZLB;
		//类别名称
		private String TZMC;
		
		public String getCpSQU() {
			return CpSQU;
		}
		public void setCpSQU(String cpSQU) {
			CpSQU = cpSQU;
		}
		public String getCPBM() {
			return CPBM;
		}
		public void setCPBM(String cPBM) {
			CPBM = cPBM;
		}
		public String getCYMC() {
			return CYMC;
		}
		public void setCYMC(String cYMC) {
			CYMC = cYMC;
		}
		public String getBJXH() {
			return BJXH;
		}
		public void setBJXH(String bJXH) {
			BJXH = bJXH;
		}
		public String getJLDW() {
			return JLDW;
		}
		public void setJLDW(String jLDW) {
			JLDW = jLDW;
		}
		public String getCBDJ() {
			return CBDJ;
		}
		public void setCBDJ(String cBDJ) {
			CBDJ = cBDJ;
		}
		public String getCpPARENTSQU() {
			return CpPARENTSQU;
		}
		public void setCpPARENTSQU(String cpPARENTSQU) {
			CpPARENTSQU = cpPARENTSQU;
		}
		public String getEDHL() {
			return EDHL;
		}
		public void setEDHL(String eDHL) {
			EDHL = eDHL;
		}
		public String getCPXH() {
			return CPXH;
		}
		public void setCPXH(String cPXH) {
			CPXH = cPXH;
		}
		public String getCPXL() {
			return CPXL;
		}
		public void setCPXL(String cPXL) {
			CPXL = cPXL;
		}
		public String getCPZM() {
			return CPZM;
		}
		public void setCPZM(String cPZM) {
			CPZM = cPZM;
		}
		public String getCPTZ() {
			return CPTZ;
		}
		public void setCPTZ(String cPTZ) {
			CPTZ = cPTZ;
		}
		public String getXHLB() {
			return XHLB;
		}
		public void setXHLB(String xHLB) {
			XHLB = xHLB;
		}
		public String getXHMC() {
			return XHMC;
		}
		public void setXHMC(String xHMC) {
			XHMC = xHMC;
		}
		public String getXLLB() {
			return XLLB;
		}
		public void setXLLB(String xLLB) {
			XLLB = xLLB;
		}
		public String getXLMC() {
			return XLMC;
		}
		public void setXLMC(String xLMC) {
			XLMC = xLMC;
		}
		public String getZMLB() {
			return ZMLB;
		}
		public void setZMLB(String zMLB) {
			ZMLB = zMLB;
		}
		public String getZMMC() {
			return ZMMC;
		}
		public void setZMMC(String zMMC) {
			ZMMC = zMMC;
		}
		public String getTZLB() {
			return TZLB;
		}
		public void setTZLB(String tZLB) {
			TZLB = tZLB;
		}
		public String getTZMC() {
			return TZMC;
		}
		public void setTZMC(String tZMC) {
			TZMC = tZMC;
		}
		public String getZP() {
			return ZP;
		}
		public void setZP(String zP) {
			ZP = zP;
		}
		public String getLXCS() {
			return LXCS;
		}
		public void setLXCS(String lXCS) {
			LXCS = lXCS;
		}
		
}
