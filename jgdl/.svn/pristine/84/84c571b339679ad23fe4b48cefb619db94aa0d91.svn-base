...
//				console.log(option.url+"page="+currPage+"&rows="+option.rowNum)
				$.ajax({
			...
			genPager:function(currPage){
				...
				if(records%rowNum>0){
			        ...
			        if(total<=showPage){
			                for (var count=1;count<=total;count++){
			                        ...
			                        if( total == count){
			                        	outstr = outstr + "<a href='javascript:void(0);' class='ck' tag='"+(currPage+1)+"'>下一页</a>";
			                        	outstr = outstr + "<a href='javascript:void(0);' class='ck' tag='"+(count)+"'>末页</a>";
			                        }
			                        }else{
			                        	outstr = outstr + "<a href='javascript:void(0);' class='ck' tag='"+(currentPage)+"'>首页</a>";
			                        	 if(total > 1){
			                        			outstr = outstr + "<a href='javascript:void(0);' class='ck' tag='"+(currPage-1)+"'>上一页</a>";
			                        		}
			   ...
			   $("#setpage-"+domId).html("<div  align='center' class='jogger'><div class='top'>"+ outstr +"</div><div class='bottom'><a href='javascript:void(0);' class='last'>当前是第"+currPage+"页  共"+total+"页/"+records+"条数据</a></div></div>");
			   outstr = "";
			    $("#"+domId+" .ck").unbind();
