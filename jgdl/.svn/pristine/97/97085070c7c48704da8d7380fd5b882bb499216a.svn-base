...
function showDeleteDoEvent(row) {
	Modal.confirm({msg : "确认删除此功能请求吗？"}).on(function(e) {
		if (e) {
			$.ajax({
				type : "post",
				url : "delAlityUrl.do",
			...
			success : function(data) {
				...
				if (data.row[0] != undefined) {
					for ( var i = 0; i < data.row.length; i++) {
						html += " <li style='display:block; padding-left:40px;' class='border-bottom'>";
						html += " <a href='javascript:void(0);'  style='color:#3f9fd9;' class='ajax-link' ";
						html += " flag='1' id='" + data.row[i].SQU + "'  tier='" + data.row[i].TIER
								+ "'  title ='" + data.row[i].NAME + "' isEnd = " + data.row[i].ISEND
								+ " onclick='parentload(this);'>";
						html += "<i class='" + data.row[i].ICON + "'></i> " + data.row[i].NAME + "</a>";
...
function showEditEvent(index) {
	...
	if (row != undefined) {
		...
		if(row.MAINSQU == 0){
			$("#placeItem").append("<option  selected='selected' value='0'></option>");
			$("#td_tag").hide();
			$("#td_sgin").hide();
		}else{
...
function addApp() {
	var flage = false;
	if($("#level").attr("value") == 0){
		$.ajax({
			url : 'addParentMenu.do',
			data: "mainName="+$("#alityName").val(),
			type : 'post',
			dataType : 'json',
			async : false,
			success : function() {
...
function loadAlityMenu(pager) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data.rows.length == 0) {
				$("#box_body").html(
					"<tr><td colspan='5'><div style='color:red;width: 100px;margin: 20px auto;'>暂无菜单功能！</div></td></tr>");
					$("#pageDiv").hide();
					top = 1;
				}
				for ( var i = 0; i < data.rows.length; i++) {
					html += " <tr>";
					html += " <td class='box_desc'><strong>"
							+ data.rows[i].NAME + "</strong>";
					html += " <p>" + data.rows[i].ENTRYURL + "</p></td>";

					html += " <td class='box_rate'>类型："
							+ data.rows[i].TYPENAME + "</td>";
					var play_text = "";
					...
					html += " <td class='box_rate'>状态：" + play_text
							+ "</td>";
					html += " <td class='box_rate'>";
					html += " <span tag='0' class='wk-uigrid'><a title='下一级' onclick='showViewEvent("
							+ i
							+ ");' href='javascript:void(0);'> <i class='icon icon-black icon-newwin'></i></a></span>&nbsp;";
					html += " <span tag='0' class='ek-uigrld'><a title='编辑' onclick='showEditEvent("
							+ i
							+ ");' href='javascript:void(0);'><i class='icon-edit icon-black'></i></a></span>&nbsp;";
					html += " <span tag='0' class='ek-uigrld'><a title='删除' onclick='showDeleteEvent("
							+ i
							+ ");' href='javascript:void(0);'><i class='icon-trash icon-black'></i></a></span>";
					html += " </td></tr>";
					$("#box_body").html(html);
